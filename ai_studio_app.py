...
(function() {{
    ...
    function setStatus(stage, title, message, percent, toastMessage) {{
        ...
        if (dot) {{
            dot.classList.toggle("active", stage !== "idle");
        }}
    ...
    function planHasOutput() {{
        return readTextbox("studio_primary_prompt").trim().length > 0;
    }}

    function watchPlanReady() {{
        if (planReadySeen || !planHasOutput()) {{
            return;
        }}
        planReadySeen = true;
        setStatus(
            "planned",
            "Plan ready",
            "Studio produced the prompt, negative prompt, workflow, and setup fields. Click Send to Engine next.",
            35,
            "Plan ready. Send it to the engine."
        );
    }}
    ...
    function postStudioPayloadToEngine(payload) {{
        ...
        if (!iframe || !iframe.contentWindow) {{
            setStatus(
                "missing-engine",
                "Engine iframe not ready",
                "Open the Hidden Fooocus engine panel or restart with START_AI_IMAGE_STUDIO.bat, then send again.",
                35,
                "Engine iframe was not ready."
            );
            return false;
        }}
        ...
        setStatus(
            "sending",
            "Sending to engine",
            "Studio is sending the prompt and negative prompt into the embedded Fooocus engine. Waiting for field-fill confirmation.",
            65,
            "Sending fields to Fooocus."
        );
        ...
        window.__fooocusStudioAckTimer = window.setTimeout(function() {{
            if (!waitingForEngineAck) {{
                return;
            }}
            setStatus(
                "sent-waiting",
                "Sent, waiting for confirmation",
                "The message was sent. If the hidden engine was still loading, open that panel and click Send to Engine again.",
                70,
                "Sent. Waiting for the engine to confirm field fill."
            );
        }}, 2200);
        return true;
    }}
    ...
    function sendStudioPlanToEngine() {{
        return postStudioPayloadToEngine(buildStudioPayloadFromFields());
    }}
    ...
    document.addEventListener("click", function(event) {{
        if (event.target.closest("#studio_build_plan_button")) {{
            ...
            setStatus(
                "planning",
                "Planning image workflow",
                "Studio is selecting the Fooocus workflow and preparing fields. This progress reflects planning only.",
                12,
                "Building plan."
            );
        }}
    ...
    window.addEventListener("message", function(event) {{
        if (event.origin !== ENGINE_ORIGIN) {{
            return;
        }}
        const payload = event.data || {{}};
        if (payload.type !== "fooocus-studio-autofill-result") {{
            return;
        }}
        ...
        if (ok) {{
            setStatus(
                "filled",
                "Engine fields filled",
                "Fooocus confirmed the prompt and negative prompt fields were filled. Open the engine panel and click Generate when ready.",
                100,
                "Engine fields filled. Click Generate when ready."
            );
        }} else {{
            setStatus(
                "partial-fill",
                "Engine needs attention",
                "Fooocus replied, but one or more target fields were not found. Open the engine panel and refresh if needed.",
                75,
                "Engine replied, but field fill was incomplete."
            );
        }}
    }});
